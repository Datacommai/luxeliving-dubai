...
import styles from "./property-listing-nearby-destinations.module.css";
...
function PropertyListingNearbyDestinations(
  ...
  const iconTypes: Record<string, string> = {
    DOWN_TOWN: "/assets/featured-properties-temp/downtown.svg",
    MARINA: "/assets/featured-properties-temp/marina.svg",
    AIRPORT: "/assets/featured-properties-temp/airport.svg",
  };

  // Filter out locations with missing data
  const validLocations = locations.filter(
    (location) =>
      ...
      location.title &&
      location.duration &&
      iconTypes[location.type] // Ensure icon type exists
  );
  ...
  return (
    <section className="w-full grid bg-[#EFEFEF] lg:grid-cols-2 xss:grid-cols-1 xxs:gap-4 md:gap-10 2xl:px-32 xl:px-24 lg:px-16 md:px-10 xxs:px-4 xxs:py-8 md:py-14 lg:py-20">
      ...
      <section className="flex flex-col justify-center text-center lg:text-left xxs:gap-3 md:gap-6">
        <span
          className={`flex xxs:justify-center lg:justify-start items-center xxs:gap-2 md:gap-6 ${styles.separatorContainer}`}
        >
        ...
        <ul className="gap-3 list-disc text-left pl-10">
...
function CloseByCard({
  ...
  return (
    <div className="flex flex-col items-start w-full h-full rounded-[6px] md:gap-6 xxs:p-3 md:py-6 md:px-4 2xl:px-8 xxs:gap-3 bg-white">
      <Image
